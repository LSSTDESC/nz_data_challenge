...
def get_data(
    ...
    ddf_path_list = [
        Path(public_data) / f"nz_challenge_{taskset}_{sim}_{scenario}_ddf_{i:02}.hdf5"
        for i in range(5)
    ]
...
def draw_rubin_mags(wfd: dict, ddf: dict) -> Figure:
    ...
    for i, band in enumerate("ugrizy"):
        axes[i][0].hist(
            ddf[f"mag_{band}_lsst"], bins=MAG_BINS, color=RUBIN_BAND_COLORS[i]
        )
        axes[i][0].set_xlabel(f"{band} [mag]")
        axes[i][0].set_ylabel("Objects / [0.1 mag]")
        axes[i][1].hist(
            wfd[f"mag_{band}_lsst"], bins=MAG_BINS, color=RUBIN_BAND_COLORS[i]
        )
...
def draw_roman_mags(wfd: dict, ddf: dict) -> Figure:
    ...
    for i, band in enumerate("YJH"):
        axes[i][0].hist(
            ddf[f"mag_{band}_roman"], bins=MAG_BINS, color=ROMAN_BAND_COLORS[i]
        )
        axes[i][0].set_xlabel(f"{band} [mag]")
        axes[i][0].set_ylabel("Objects / [0.1 mag]")
        axes[i][1].hist(
            wfd[f"mag_{band}_roman"], bins=MAG_BINS, color=ROMAN_BAND_COLORS[i]
        )
        axes[i][1].set_xlabel(f"{band} [mag]")
        axes[i][1].set_ylabel("Objects / [0.1 mag]")
    fig.tight_layout()
