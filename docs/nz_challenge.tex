...
per-object $p(z)$ distribuitons in many ways.   Depending on the method,
estimating $n(z)$ distributions may or may not include doing $p(z)$ estimation along the way.
...
first part, the PZ data challenge, assesssing per-object $p(z)$ estimation,
was launched in April, 2026, and will concluded in September, 2026.
The second part, tomography and $n(z)$ estimation, will focus on assigning
objects to tomographic bins and estimating the distribution of redshifts in
each bin, and will launch in September 2026.

Preliminary results will be released in January, 2027, with a technical note
summarizing those results to follow shortly thereafter and
...
The \texttt{nz\_data\_challenge} package will provide participants with tools
...
\texttt{nz\_data\_challenge} provided a command line interface to
...
\begin{enumerate}
\item{Providing tomographic bin assignments for all objects, and estimate ensemble $n(z)$
  ...
  \item{Provide trained models for the different scenarios and a
      ...
      1 on an arbitrary dataset.   We will not actually run these ourselves as part of the challenge,
      but we do ask that you providing timing estimates for how long it took you to run this.}
  \item{Provide a Python function that can be used to
      generate the models and estimates from subtasks 1 and 2 on
      arbitrary datasets.   We will not actually run these ourselves as part of the challenge,
      but we do ask that you providing timing estimates for how long it took you to run this.}
\end{enumerate}

The $n(z)$ estimates in subtask 1 and the timing estiamtes should be provided
in a compressed \texttt{tar} file.   Any models used in subtask 2 should be provided
seperately.  Templates and instructions for the Python functions needed for subtasks 2 and 3 will be provided and are
described below. 


\subsection{Data format for timing information}

The timing information should be provided as a yaml file containing a nested dictionary, named \texttt{timing.yaml'}.
All times should be given in seconds.   Here is an example:
...
\subsection{Data format for bin assignments}

The bin assignements should be provided as an hdf5 file containing a dictionary
with two arrays: object ids (\texttt{object\_id}) and corresponding bin assignments (\texttt{bhat\_for\_wide\_data}).   The tomographic
bins should run from 0 to $n_{\rm bins} -1$.   The number $-1$ is reserverd as a guard
value for unassigned objects.   The files should be called \texttt{nz\_challenge\_taskset\_1\_cardinal\_\_bhat\_1yr.hdf5}.
...
\subsection{\texorpdfstring{Data format for ensemble  $n(z)$ estimates}{Data format for ensemble n(z) estimates}}
...
the $n(z)$ as either an histogram grid or a Gaussian mixture model.
The files should be called \texttt{nz\_challenge\_taskset\_1\_cardinal\_\_nz\_estimate\_1yr.hdf5}.
...
\subsection{Packaging submission files}
...
\texttt{nz\_estimate} (for the $n(z)$ estimates),
\texttt{bhat\_estimate} (for the bin assignements) and will be specified in
...
The timing information file should simply be called \texttt{timing.yaml}.
All of these files should then be joined into a compressed \texttt{tar} file, which 
should then be placed somewhere it can be download.  The URL for the
\texttt{tar} should be specified in
\texttt{tests/test\_\{submission\}.py}.   Please take care to avoid putting extra directory
structure in the tar file.  I.e., we would like to uncompress it directly to the target area.
...
def run_taskset_1_estimation_only(
    ...
    # do stuff and write n(z) estimates to "output_nz_file"
...
def run_taskset_1_training_and_estimation(
    ...
    # train a model using the "ddf_files" (and possibley wfd_file) 
...
\texttt{bz\_data\_challenge} repository and will include:
...
give the location of the \texttt{tar} file containing the NZ estimates, bin assignemnts,
timing estimates and trained models, and to implement the required functions.

See \texttt{https://github.com/LSSTDESC/nz\_data\_challenge/pull/6}
...
\begin{enumerate}
    ...
    \item{the $bhat$ file exists;}
    \item{the $bhat$ file contains a two-column table with ``bhat'' assignement and ``object\_id'' for each object.}
...
assigns objects to the desired tomographic bin are based on the confusion
matrix.  I.e., the matrix $N_{\rm true, \rm assigned}$.   In the ideal case this
...
\begin{itemize}
    \item \texttt{Accuracy} is simply the median of $\Delta_i$.
    \item \texttt{Balanced Accuracy} is the fraction of the $\Delta$ distribution
      outside of $[0, {\rm max}(0.06, 3\sigma_{\rm iqr})]$.
    \item \texttt{Cohen's Kappa} .
    \item \texttt{Mutual Information} .
    \item \texttt{Log Loss From Labels} .
\end{itemize}
...
accurate estimate of the posterior distribution, $n(z)$, for each
ensemble following metrics.

\begin{itemize}
    \item{\texttt{Total information Loss}.}  
    \item{\texttt{Wasserstein Distance}.}
    \item{\texttt{RMS of $\delta_{\rm true} - \delta_{\rm est}$}.}
    \item{\texttt{RMS of $\sigma_{\rm true} - \sigma_{\rm est}$}.}     
\end{itemize}



\subsection{Metrics for cosomology analysis (Fisher forecasts)}

We will also estimate how well each algorithm would perform in the context
of cosmological analyses.  
...
The first, simplest task is to assign objects to fixed tomographic bins, and estimate ensemble PDFs using
largely representative training samples. I.e., the training samples are drawn
from the essentially the same distributions as the test samples.   In fact, both the training
data and the test data are drawn from a sample that includes 
some mislabeled many-band photometric redsfhits and  “unrecognized blends” multiple object
that are detected as a single object by the image
processing algorithm, however these
primarily effect fainter objects, and for this task set we
simply a uniform magnitude cut of $i < 23$ in selecting objects for both
...
redshifts to serve as labels for most objects, and many-band that includes 
some mislabeled many-band photometric for all objects.
Each file has the statsitics typical of a single
deep-drilling field.
...
\begin{enumerate}
  \item{Assign each object in this file to a tomographic,  estimate the $n(z)$ distribution for each bin.
      provide bin assignemtns and estimates as part of the downloadable \texttt{tar} file.}
  \item{Provide pre-trained models appropriate to each of the training
      ...
      pre-trained models to assign to assign each object in this file to a tomographic bin and then estimate the $n(z)$
  ...
  \item{Implement a Python function (\texttt{run\_taskset\_1\_training\_and\_estimation}) to train a model for
      each training set and use that model to assign each object in this file to a tomographic bin and then estimate the $n(z)$
      distribution for each bin.}
\end{enumerate}
...
data and the test data are drawn from a sample that includes 
some mislabeled many-band photometric redsfhits and  “unrecognized blends”, but here
we retained all the objects down to $i < 25.5$ in the both the test and training sets.
However, since the training set includes the emulation of spectroscopic selections, it will not be
representative of the fainter objects in the test set. This reflects that spectroscopic redshifts
...
\subsection{Taskset 3: Assign objects to fixed tomographic bins
  and estimate ensemble PDFs and the uncertainty on those PDFs from cosmic-variance
  using non-representative samples}

This task is almost identical to taskset 2, the only difference is that we are asking you to include estimates
on the uncertainty of the 

This taskset re-uses the files form taskset 2.

The subtasks in this task set are:

\begin{enumerate}
\item{Assign each object in this file to a tomographic,  estimate the $n(z)$ distribution for each bin and provide
    a set of ``equally'' probably $n(z)$ realizations for each tomographic bin that properly account for the effects of
...
In the files for tasksets 1-3 we emulate the effect of unrecognized
...
to emulate the ``calibration'' or ``deep-drilling'' fields (DDFs) and selected a sample
for the entire footprint (WFD) to serve as a test dataset.
...
\section{Data Preperation tools}
\label{sec:data_prepration_tools}
