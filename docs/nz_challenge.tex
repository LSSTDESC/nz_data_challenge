...
first part, the PZ data challenge, assessing per-object $p(z)$ estimation,
...
Preliminary results for the NZ part of the challenge will be released in January 2027,
...
\item{Provide tomographic bin assignments for all objects and estimate ensemble $n(z)$
  ...
  \item{Provide trained models for the different scenarios and a
      ...
      but we do ask that you provide timing estimates for how long it took you to run this.}
...
The $n(z)$ estimates in subtask 1 and the timing estimates should be provided
in a compressed \texttt{tar} file.  Any models used in subtask 2 should be provided
separately.  Templates and instructions for the Python functions needed for subtasks 2 and 3 are
described below. 

\subsection{Data format for submissions}

\subsubsection{Data format for timing information}
...
\texttt{nz\_estimate} (for the $n(z)$ estimates) and
\texttt{bhat\_estimate} (for the bin assignments), and will be specified in
...
should then be placed somewhere it can be downloaded.  The URL for the
\texttt{tar} file should be specified in
...
You will need to modify the \texttt{tests/test\_\{submission\}.py} to
give the location of the \texttt{tar} file containing the $n(z)$ estimates, bin assignments,
...
\begin{enumerate}
    ...
    \item{the $bhat$ file contains a two-column table with ``bhat'' assignment and ``object\_id'' for each object.}
...
\begin{itemize}
  ...
  \item{\texttt{scripts/run\_metrics.py}:  runs performance metrics on
...
\texttt{fisherA2Z} package.  For each submission we compute the
$w_0$--$w_a$ dark energy Figure of Merit (FoM), defined as the inverse
area of the $1\sigma$ contour in the $w_0$--$w_a$ plane, under two
analysis configurations:

\begin{itemize}
    \item \textbf{Cosmic Shear FoM}: A Fisher forecast using only the
      cosmic shear two-point correlation function.
    \item \textbf{3$\times$2pt FoM}: A Fisher forecast using the full
      combination of cosmic shear, galaxy--galaxy lensing, and galaxy
      clustering (the ``3$\times$2pt'' data vector).
\end{itemize}

These forecasts use the submitted $n(z)$ estimates and the true
$n(z)$ distributions to quantify the bias in cosmological parameters
induced by errors in the redshift distributions.  The forecast
...
performance of the narrow-band photometric redshifts is shown in Fig.~\ref{fig:manyband_photoz}.
